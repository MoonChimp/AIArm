...
def main():
    ...
    try:
        ...
        # Check if we should process the input as JSON
        input_text = args.input
        agent_name = args.agent
        user_id = args.user
        
        # If the --json flag is set, parse the input as JSON
        if args.json and input_text:
            try:
                input_json = json.loads(input_text)
                input_text = input_json.get("input", input_text)
                agent_name = input_json.get("agent", agent_name)
                user_id = input_json.get("user_id", user_id)
